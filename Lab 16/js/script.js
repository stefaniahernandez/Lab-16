...
var calculateDays = function () {
    ...
    if (event.length === 0 || dt.length === 0) {
        ...
        return;   //Need return to stop the function
    }
    ...
    //CONVERT THE DATE TO A DATE OBJECT AND MAKE SURE IT IS VALID
    date = new Date(dt);
    ...
    //IF DATE HAS PASSED
    if (days < 0) {
        $("message").innerHTML = event + " already happened " + Math.abs(days) + " days ago." + date.toDateString();
    }
    //IF DATE IS IN THE FUTURE
    if (days > 0) {
        $("message").innerHTML = days + " days until " + event + ". " + date.toDateString();
    }
